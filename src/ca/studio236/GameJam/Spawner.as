package ca.studio236.GameJam
{
	...
	public class Spawner
	{
		...
		public function tickSpawner() {
			...
			if(score._score >= 250000 && !bossHasSpawned) {
				PlayState.boss = true;
				bossHasSpawned = true;
			}
			if(Math.random() < 0.00001) {
				...
				else if(zx > 0.25 && zx <= 0.50) {
					_group.add(new CholDerb(c,100,Math.floor(Math.random()*FlxG.width), FlxG.height + 128));//bottom
				}
				else if(zx > 0.50 && zx <= 0.75) {
					_group.add(new CholDerb(c,100,-128, Math.floor(Math.random()*FlxG.height)));//left
				}
				else if(zx > 0.75) {
					_group.add(new CholDerb(c,100,FlxG.width + 128, Math.floor(Math.random()*FlxG.height)));//right
				}
			}
			...
			if(spawnevent < 0.0001 && spawnstate == "normal" && score._score > 50000) {
				
				//PLAQUE ATTACK
				this.spawnstatetimer = 1000;
				this.spawnstate = 'plak_attack';
				_hud.add(new TextBlast(_hud,"PLAQUE ATTACK!!!"));
				FlxG.play(ALARM);
			...
			if(spawnstate == 'plak_attack' && spawnstatetimer > 0) {
				...
				if(this.spawnstatetimer % 65 == 0) {
					...
					if(pa <= 0.25) {
						...
						_group.add(new Plaque(c,-32, Math.floor(Math.random()*FlxG.height)));//right
					}
					else if(pa > 0.75) {
						
						_group.add(new Plaque(c,FlxG.width + 32, Math.floor(Math.random()*FlxG.height)));//left
					}
				}
					...
					while(_group.length < maxEnemies) {
						...
						if(score._score < 30000) {
							...
							if(z1 <= 0.25) {
								...
								_group.add(new CholSmall(c,100,-32, Math.floor(Math.random()*FlxG.height)));//left
							}
							else if(z1 > 0.75) {
								_group.add(new CholSmall(c,FlxG.width + 32, Math.floor(Math.random()*FlxG.height)));//left
							}
							
						}
						//Zone 2
						else if(score._score < 75000){
							var z2 = Math.random();
							var z22 = Math.random();
							if(z2 <= 0.25) {
								if(z22 < 0.50) {
								...
								if(z22 < 0.50) {
									...
									_group.add(new CholBig(c,100,-32, Math.floor(Math.random()*FlxG.height)));//left
								}
							}
							else if(z2 > 0.75) {
								if(z22 < 0.50) {
									_group.add(new CholSmall(c,100,FlxG.width + 32, Math.floor(Math.random()*FlxG.height)));//right
								}else{
									_group.add(new CholBig(c,100,FlxG.width + 32, Math.floor(Math.random()*FlxG.height)));//right
								}
							}
						}
						...
						//Zone 3
							...
							if(z3 <= 0.25) {
								...
								if(z33 < 0.50) {
									...
									_group.add(new Plaque(c,-32, Math.floor(Math.random()*FlxG.height)));//left
								}
							}
							else if(z3 > 0.75) {
								if(z33 < 0.50) {
						...
						//zone 4
							...
							if(z4 <= 0.25) {
								...
								if(z44 < 0.40) {
									...
									_group.add(new PlaqueDaddy(b,c,-32, Math.floor(Math.random()*FlxG.height)));//left
								}
							}
							else if(z4 > 0.75) {
								if(z44 < 0.50) {
									...
									_group.add(new Plaque(c,FlxG.width + 32, Math.floor(Math.random()*FlxG.height)));//right
								}else{
									_group.add(new PlaqueDaddy(b,c,FlxG.width + 32, Math.floor(Math.random()*FlxG.height)));//right
								}
							}
						}
						...
						//final zone
							...
							if(z5 <= 0.25) {
								...
								if(z55 < 0.40) {
									...
									_group.add(new HeartWorm(c,-32, Math.floor(Math.random()*FlxG.height)));//left
								}
							}
							else if(z5 > 0.75) {
								if(z55 < 0.50) {
									...
									_group.add(new HeartWorm(c,FlxG.width + 32, Math.floor(Math.random()*FlxG.height)));//right
								}
							}
